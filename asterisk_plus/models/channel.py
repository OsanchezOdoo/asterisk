...
class Channel(models.Model):
    ...
    def update_call_data(channel, country=None):
        ...
        call_data = {}
        if channel.user:
            if len(channel.call.channels) == 1: # This is the primary channel.
            ...
            if not channel.call.partner:
                ...
                if not call_data['partner'] and call_data['direction'] == 'in' and \
                        channel.env['asterisk_plus.settings'].get_param('auto_create_partners'):
                    call_data['partner'] = channel.env['res.partner'].sudo().create({
                        'name': channel.callerid_num,
                        'phone': channel.callerid_num,
                    }).id
                    debug(self, 'Call {} auto create partner id {}'.format(
        ...
        try:
            if channel.call and not self.call.ref:
    ...
    def on_ami_new_channel(self, event):
        ...
        if not channel.user:
            ...
            if user_channel:
                data['user'] = user_channel.id
        else:
            user_channel = channel.user
        # Define country for number formatting
        country = (user_channel.partner_id.country_id.code or
            self.env.user.partner_id.country_id.code or None
        )
        debug(self, '{} id {} user {} country {}'.format(
            event['Channel'], channel.mapped('id'), user_channel.id, country
        ))
    ...
    def on_ami_update_channel_state(self, event):
        ...
        if channel.call and get('ChannelStateDesc') == 'Up':
            user_channel = self.env['asterisk_plus.user_channel'].get_user_channel(
                channel.channel, channel.system_name)
            call_data = {
                'status': 'answered',
                'answered': datetime.now(),
            }
            if user_channel and len(channel.call.channels) > 1:
                # Re-write called user field.
                call_data['called_user'] = user_channel.sudo().asterisk_user.user.id
            channel.call.write(call_data)
        if channel.call:
